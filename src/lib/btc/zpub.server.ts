/**
 * BIP84 zpub → native SegWit mainnet address. Server-only — never import from client.
 * Uses @scure/@noble only (no bitcoinjs ECC init) for reliable Node/Vite builds.
 *
 * Mainnet only. Testnet vpub/tpub derivation was removed.
 */
...
function toXpub(extended: string): string {
  ...
  if (!altOk) {
    throw new Error("Expected a mainnet zpub (or xpub).");
  }
  const next = new Uint8Array(decoded.length);
  next.set(XPUB_VERSION, 0);
...
/**
 * Derive external-chain native SegWit address at m/0/i from account-level zpub.
 * zpub is already at m/84'/0'/0'. Mainnet bech32 (bc1) only.
 */
export function deriveAddress(zpub: string, index: number): string {
  if (!zpub?.trim()) throw new Error("Bitcoin zpub is not configured.");
  if (!Number.isInteger(index) || index < 0 || index > 2_147_483_647) {
    throw new Error("Invalid derivation index.");
  }
  const xpub = toXpub(zpub);
  ...
  return p2wpkhAddress(child.publicKey);
}
